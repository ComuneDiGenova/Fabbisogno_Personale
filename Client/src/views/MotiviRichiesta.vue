  ...
  data() {
        return {
            ...
            columns: [
                ...
                {
                    field: 'NOME',
                    label: 'Motivo richiesta',searchable: true,
                    sortable: true
                }
            ]
        }
  ...
  methods: {
    doDelete() {
      this.$buefy.dialog.confirm({
        ...
        onConfirm: () => {
          const loading = this.$buefy.loading.open();
          api.motiviRichiesta.delete(this.newItem.ID) 
            .then (()=>{
              return api.motiviRichiesta.getAll()
            })
            .then ((rows)=>{
              this.rowData = rows.data;
              helpers.ok('Motivo richiesta correttamente cancellato');
    ...
    doEdit() {
      ...
      let apiMethod = this.newItem.ID!=0?api.motiviRichiesta.update:api.motiviRichiesta.create;
      const loading = this.$buefy.loading.open();
      ...
      .then ((rows)=>{
          this.rowData = rows.data;
          helpers.ok('Motivo richiesta correttamente salvato');
          this.newItem = emptyVal;
          this.selectedTab = 0;
          loading.close();
      })
